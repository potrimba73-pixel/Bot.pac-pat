...
import { logExternalSearch } from "./externalLogs.js";
...
export async function handleAjudaCommand(interaction, client) {
    ...
    const descricao = [
        ...
        "ETS2LA — Configuracao, mods, atualizacoes",
        ...
        ...(recentes.length > 0 ? ["", "Perguntas recentes:", ...recentes.map((r, i) => {
    ...
    const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId("ajuda_procurar").setLabel("Procurar ajuda").setStyle(ButtonStyle.Primary),
        new ButtonBuilder().setCustomId("ajuda_ticket").setLabel("Abrir ticket").setStyle(ButtonStyle.Danger),
    );
...
export async function handleAjudaModal(interaction, client) {
    ...
    if (!isTopicoPermitido(pergunta)) {
        const embed = new EmbedBuilder()
            ...
            .setTimestamp();
        const row = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId("ajuda_nova").setLabel("Nova pergunta").setStyle(ButtonStyle.Primary),
            new ButtonBuilder().setCustomId("ajuda_ticket").setLabel("Abrir ticket").setStyle(ButtonStyle.Danger),
        );
        await interaction.editReply({ embeds: [embed], components: [row] });
        return;
    ...
    if (tutorial) {
        assistantMemory.recentHelp.set(interaction.user.id, { pergunta: pergunta, resposta: tutorial.titulo, timestamp: Date.now() });
        const descricao = [
            tutorial.resumo,
            "",
    ...
    assistantMemory.recentHelp.set(interaction.user.id, { pergunta: pergunta, resposta: resposta.titulo, timestamp: Date.now() });
    if (resposta.found) {
        const embed = new EmbedBuilder()
            .setTitle(resposta.titulo)
            .setDescription(resposta.texto(CONFIG))
            .setColor(0x00ff00)
            .setFooter({ text: "Informacao automatica — pode nao estar 100% atualizada" })
            .setTimestamp();
        const row = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId("ajuda_nova").setLabel("Nova pergunta").setStyle(ButtonStyle.Primary),
            new ButtonBuilder().setCustomId("ajuda_ticket").setLabel("Abrir ticket").setStyle(ButtonStyle.Danger),
    ...
    // Log para servidor externo
    await logExternalSearch(interaction.user, pergunta);
    const embed = new EmbedBuilder()
        ...
        .setDescription([
            ...
            "• Verificar os tutoriais no canal de regras",
