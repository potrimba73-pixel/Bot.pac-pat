...
async function getExternalLogs() {
    if (!externalLogs) {
        try {
            ...
            try {
                externalLogs = await import("./src/src/services/externalLogs.js");
            } catch (e2) {
                return null;
            }
        }
...
async function safeImport(path) {
    try {
        ...
        // Tentar caminho alternativo (src/src/)
        const altPath = path.replace("./src/", "./src/src/");
        try {
            return await import(altPath);
        } catch (e2) {
            console.log("[Import] Nao encontrado:", path, "nem", altPath);
            return {};
        }
    }
}
...
client.once(Events.ClientReady, async () => {
    ...
    if (ext && ext.setExternalClient) ext.setExternalClient(client);
