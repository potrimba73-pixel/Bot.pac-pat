...
export async function handleGuildMemberRemove(member, client) {
    ...
    if (db.acceptedRules.includes(userId)) {
        ...
        saveDB();
        console.log(`🗑️ Registo de regras removido para ${member.user.tag}`);
    }
    ...
    for (const ticket of userTickets) {
        ...
        try {
            ...
            if (ticketChannel) {
                ...
                console.log(`🗑️ Ticket de ${member.user.tag} apagado`);
            }
        } catch (e) {
            console.log(
                `Não foi possível apagar ticket de ${member.user.tag}:`,
                e.message,
            );
        }
    }
    ...
    if (userTickets.length > 0) {
        saveDB();
        console.log(
            `🎫 ${userTickets.length} ticket(s) de ${member.user.tag} fechado(s)`,
        );
    }
}
