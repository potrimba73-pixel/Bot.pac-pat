...
export async function handleInteractionCreate(interaction, client) {
    ...
    if (interaction.isModalSubmit()) {
        if (interaction.customId.startsWith("modal_avaliar_")) {
            ...
            saveDB();
            await safeEditReply(interaction, { content: `Obrigado pela sua avaliação de ${estrelas} estrelas!`, flags: 64 });
            return;
        }
    ...
    if (interaction.isChatInputCommand()) {
        ...
        if (interaction.commandName === "limpar") {
            if (!interaction.member.permissions.has(PermissionFlagsBits.ManageMessages)) {
                return interaction.reply({ content: "Não tens permissão para usar este comando.", flags: 64 });
            }
        ...
        if (interaction.commandName === "painelstaff") {
            ...
            if (!ticket) {
                return interaction.reply({ content: "Este canal não é um ticket ativo.", flags: 64 });
            }
            ...
            const panelText = [
                ...
                `Tipo: ${ticket.label}`,
                `Assumido por: ${ticket.claimedBy ? ticket.claimedByName : "Ninguém"}`,
        ...
        if (interaction.commandName === "passar") {
            ...
            if (ticket.claimedBy && ticket.claimedBy !== interaction.user.id) {
                return interaction.reply({ content: "Só quem assumiu o ticket pode passá-lo. Usa /pedirassumo primeiro.", flags: 64 });
            }
            ...
            if (!targetMember || !targetMember.roles.cache.has(CONFIG.CARGO_STAFF)) {
                return interaction.reply({ content: "O utilizador selecionado não é staff.", flags: 64 });
            }
            await interaction.deferReply({ flags: 64 });
        ...
        if (interaction.commandName === "pedirassumo") {
            ...
            if (!ticket.claimedBy) {
                ...
                saveDB();
                await updateTicketEmbed(interaction.channel, ticket.id);
                await interaction.reply({ content: "Ticket assumido! Ninguém tinha assumido ainda.", flags: 64 });
                await interaction.channel.send(
            ...
            if (ticket.claimedBy === interaction.user.id) {
                return interaction.reply({ content: "Já assumiste este ticket!", flags: 64 });
            }
            ...
            if (!currentStaff) {
                ...
                saveDB();
                await updateTicketEmbed(interaction.channel, ticket.id);
                await interaction.reply({ content: "O staff anterior não foi encontrado. Ticket assumido por ti!", flags: 64 });
                return;
            }
            await interaction.deferReply({ flags: 64 });
            try {
                ...
                const row = new ActionRowBuilder().addComponents(
                    ...
                    new ButtonBuilder().setCustomId(`recusar_assumo_${ticket.id}_${interaction.user.id}`).setLabel("Não, Ficar").setStyle(ButtonStyle.Danger),
                );
...
if (customId.startsWith("aceitar_regras_rec_")) {
  ...
  return;
        ...
        if (customId.startsWith("recusar_regras_rec_")) {
            ...
            if (interaction.user.id !== userId) {
                return interaction.editReply({ content: "Não podes recusar por outra pessoa!", components: [] });
            }
            await interaction.editReply({
                content: "Recusaste as regras. Não foi criado nenhum ticket de recrutamento.",
                components: [], embeds: [],
            });
            return;
        }
        ...
        if (customId === "aceitar_regras") {
            ...
            if (jaAceitou && temCargoMembro) {
                return safeEditReply(interaction, { content: "Já aceitaste as regras e tens o cargo atribuído!", flags: 64 });
            }
            try {
                ...
                await safeEditReply(interaction, { content: "Ocorreu um erro ao processar. Tenta novamente ou contacta a staff.", flags: 64 });
            }
            return;
        }
        ...
        if (customId.startsWith("sair_")) {
            ...
            if (ticket && ticket.userId === interaction.user.id) {
                ...
                await safeEditReply(interaction, { content: "Saíste do ticket. Podes fechá-lo se desejares.", flags: 64 });
            } else {
        ...
        if (customId.startsWith("assumir_")) {
            ...
            if (ticket.claimedBy) {
                return safeEditReply(interaction, { content: `Este ticket já foi assumido por ${ticket.claimedByName}.`, flags: 64 });
            }
            ticket.claimedBy = interaction.user.id;
            ticket.claimedByName = interaction.user.username;
            saveDB();
        ...
        if (customId.startsWith("deletar_")) {
            ...
            if (ticket.type === "recrutamento") {
                ...
                ticket.closed = true;
                saveDB();
                await safeEditReply(interaction, { content: "Ticket de recrutamento aguarda decisão da staff." });
                return;
            }
            ...
            await safeEditReply(interaction, { content: "Ticket será fechado em 10 segundos..." });
            setTimeout(async () => {
        ...
        if (customId.startsWith("avaliar_")) {
            ...
            if (db.avaliacoes[ticketId] && db.avaliacoes[ticketId].some(a => a.avaliador === interaction.user.id)) {
                return interaction.reply({ content: "Já avaliaste este ticket!", flags: 64 });
            }
        ...
        if (customId.startsWith("add_user_")) {
            ...
            const ticket = db.tickets[ticketId];
            if (!ticket) return;
            await interaction.reply({ content: "Para adicionar um usuário, menciona-o neste canal e um staff pode adicionar manualmente nas permissões.", flags: 64 });
            return;
        }
        ...
        if (customId.startsWith("remove_user_")) {
            ...
            const ticket = db.tickets[ticketId];
            if (!ticket) return;
            await interaction.reply({ content: "Para remover um usuário, um staff pode remover manualmente nas permissões do canal.", flags: 64 });
            return;
        }
    }
}
