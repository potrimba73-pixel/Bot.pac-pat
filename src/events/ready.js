...
export async function handleReady(client) {
  ...
  try {
    ...
    if (externalGuild) {
      ...
      console.warn("[Ready] Servidor externo de logs nao encontrado. Verifica se o bot esta no servidor 1510401803974475947");
    }
  } catch (err) {
  ...
  try {
    ...
    if (!mainGuild) {
      ...
      console.log("[Ready] Servidor principal encontrado:", mainGuild.name);
      ...
      if (canalGeral) {
        ...
        if (painelExiste) {
          try {
            ...
            console.log("[Ready] Painel geral ja existe");
          } catch {
      ...
      if (canalRegras) {
        ...
        if (regrasExiste) {
          try {
            ...
            console.log("[Ready] Painel de regras ja existe");
          } catch {
  ...
  try {
    if (!CONFIG.GUILD_ID_RECRUTAMENTO) {
      console.log("[Ready] GUILD_ID_RECRUTAMENTO nao definido, a ignorar servidor de recrutamento");
      return;
    }

    const recGuild = await client.guilds.fetch(CONFIG.GUILD_ID_RECRUTAMENTO).catch(() => null);
    if (!recGuild) {
      console.warn("[Ready] Servidor de recrutamento nao encontrado. Verifica se o bot esta no servidor:", CONFIG.GUILD_ID_RECRUTAMENTO);
      return;
    }

    console.log("[Ready] Servidor de recrutamento encontrado:", recGuild.name);
    ...
    if (canalRec) {
      ...
      if (painelRecExiste) {
        try {
          ...
          console.log("[Ready] Painel de recrutamento ja existe");
        } catch {
    ...
    if (canalRegrasRec) {
      ...
      if (regrasRecExiste) {
        try {
          ...
          console.log("[Ready] Painel de regras recrutamento ja existe");
        } catch {
...
async function enviarPainelRegras(canal) {
  const embed = new EmbedBuilder()
    .setTitle(`${CONFIG.EMOJI_REGRAS} Regras | Portugal Alfa Community`)
    .setDescription([
      `${CONFIG.EMOJI_INFO} Le atentamente as regras antes de aceitares.`,
      "",
      "Clica no botao abaixo para aceitar as regras e obteres acesso ao servidor.",
    ].join("\n"))
...
async function enviarPainelRegrasRecrutamento(canal) {
  const embed = new EmbedBuilder()
    .setTitle(`${CONFIG.EMOJI_REGRAS} Regras | Portugal Alfa Truckers`)
    .setDescription([
      `${CONFIG.EMOJI_INFO} Le atentamente as regras antes de aceitares.`,
      "",
