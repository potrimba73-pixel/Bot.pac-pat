...
export async function chamarMembro(interaction, ticketId, client) {
    ...
    try {
        ...
        if (!user) {
            return safeEditReply(interaction, {
                content: "❌ Não foi possível encontrar o utilizador.",
                flags: 64,
            });
        }

        // Create stylish DM embed
        const embed = new EmbedBuilder()
            .setTitle("📢 Staff a Chamar!")
            .setDescription([
                `Olá <@${ticket.userId}>!`,
                "",
                `👋 Um membro da staff está a chamar-te no teu ticket **#${ticketId}**.`,
                "",
                `📋 **Motivo:** ${ticket.label}`,
        ...
        const row = new ActionRowBuilder().addComponents(
            ...
            new ButtonBuilder()
                .setCustomId(`reentrar_ticket_${ticketId}`)
                .setLabel("🎫 Re-entrar no Ticket")
                .setStyle(ButtonStyle.Primary)
        );

        await user.send({ embeds: [embed], components: [row] });

        // Also send notification in ticket channel
        await interaction.channel.send({
            content: `📢 <@${interaction.user.id}> chamou <@${ticket.userId}> no privado.`,
            allowedMentions: { users: [ticket.userId] }
        });

        await safeEditReply(interaction, {
            content: `✅ Mensagem enviada para <@${ticket.userId}> no privado!`,
            flags: 64,
        });
        ...
        await safeEditReply(interaction, {
            content: "❌ Erro ao enviar mensagem no privado. O utilizador pode ter DMs desativadas.",
            flags: 64,
        });
    }
}
